...
from gensim.models import KeyedVectors # Loads pre-trained word embeddings like Word2Vec
import random
...
class NgramDataset(Dataset):
    def __init__(self, data, vocab, context_size=5, pad_token='<PAD>'):
        ...
        self.pad_index = self.vocab.get(pad_token, len(vocab)) # Retrieves the index of the padding token ('<PAD>') from vocab
        self.data = self._process_data(self.data) # Preprocess the sentences into context-target pairs
    ...
    def _process_data(self, data):
        ...
        for sentence in data:
            ...
            # Adds a single <END> token at the end
            ...
            for i in range(len(sentence) - self.context_size):
                ...
                if self.vocab['<END>'] in context:
                    continue
                processed_data.append((context, target)) # Adds the context and target pair to the processed_data list

        return processed_data

    def __len__(self): # Returns the number of context-target pairs in the dataset
        return len(self.data)

    def __getitem__(self, idx): # Retrieves the idx-th context-target pair from self.data
...
# Maps words to pre-trained embeddings from GloVe and initializes random embeddings for unknown words
...
for word, idx in vocab.items():
    if word in glove_vector:
        ...
        wt_matrix[idx] = np.random.normal(scale=0.2, size=embedding_dim) # Generates a random vector with mean 0 and sd 0.2
...
class NeuralLanguageModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, context_size=5,
        ...
        # If no pre-trained matrix is provided, a new embedding layer is created with random initialization
        
        self.fc1 = nn.Linear(hidden_dim * context_size, hidden_dim)
        self.dropout = nn.Dropout(dropout_rate) # Applies dropout with the given rate to prevent overfitting
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1) # Batch size, number of columns
...
model = NeuralLanguageModel(vocab_size, embedding_dim, hidden_dim, context_size, wt_matrix, dropout_rate=0.5, padding_idx=padding_idx).to(device)
...
def train(model, train_loader, val_loader, criterion, optimizer, epochs=1, scheduler=None):
    for epoch in range(epochs):
        ...
        for context, target in train_loader:
            ...
            loss.backward() # Computes the gradients of the loss with  model parameters

            # Gradient clipping
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0) # Prevents exploding gradients in case of large updates
        ...
        # Validation
        model.eval() # Disable layers like dropout or batch normalization
        val_loss = 0
        with torch.no_grad():
...
class LSTMModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, embedding_matrix):
        ...
        self.embedding = nn.Embedding.from_pretrained(embedding_matrix, freeze=False) # Allows the embeddings to be fine-tuned during training
        ...
        self.softmax = nn.Softmax(dim=-1) # Softmax is applied along the last dimension
